...
export async function POST(_request: NextRequest, context: RouteContext) {
  try {
    const rateLimited = await checkRateLimit("project-team-decline", 20);
    ...
    if (error) {
      console.error("[api] project team decline error:", error);
      return NextResponse.json(
    ...
    console.error("[api] project team decline handler error:", err);
    return NextResponse.json(
